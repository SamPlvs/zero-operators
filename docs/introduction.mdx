...
<Tip>
  **Cost-conscious?** Pass `--low-token` to `zo build` (or set `low_token: true` in plan frontmatter) to swap the lead orchestrator from Opus to Sonnet, cap Phase-4 iterations at 2, and disable several auxiliary calls. ~70-80% reduction in token spend at a slight quality trade. See [low-token mode](/concepts/low-token-mode).
</Tip>
