...
Isolation is structural: a per-PID lock file lets the platform detect concurrent sessions, avoid disturbing a live peer's permission overlay, and auto-consolidate when the last session exits. A report session never touches the shared permission overlay (only orchestrator sessions own it), so it cannot disrupt a running `zo continue`. If a model session was started *before* this feature shipped it isn't in the registry, so auto-merge can't detect it — pass `--no-consolidate` to that report run, then merge later with `zo consolidate` once the model session has ended.
