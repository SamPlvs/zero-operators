...
To break past the ~30% ceiling without losing quality, the path forward is architectural — moving ZO from `claude` CLI subprocess to direct Anthropic SDK so ZO can control caching, batching, and file uploads programmatically:
...
| **Sub-agent model right-sizing** | Already done for Sonnet sub-agents. Could push code-reviewer / test-engineer / oracle-qa down to Haiku 4.5 for pattern-matching work | **~10-15%** (Haiku is ~3× cheaper than Sonnet) | Update agent `.md` frontmatter |
| **Phase-1 trim in low-token mode** | Phase 1 was ~45% of bench cost (\$3.47 of \$7.75). Run with data-engineer only; defer review/tests to Gate 5 | **~10-15%** | Orchestrator `_agents_for_phase` change |

Combined with the existing 30% headline, these levers would realistically land low-token at **50-60%** without an SDK refactor (the last two rows alone add ~20-30 pp), and **70-80%** with the SDK refactor (the first three rows). The 70-80% figure was always achievable — just not via the v1 preset alone.
