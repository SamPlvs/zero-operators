...
> **Autonomous iteration loop (ZO infrastructure):** in `auto` and `full_auto` gate modes, Phase 4 runs as a closed loop. After the Oracle's `result.md` is parsed at a gate, the orchestrator consults the loop evaluator (`zo.experiment_loop`). Verdicts: (a) **`TARGET_HIT`** — latest exp hits `oracle_tier >= policy.stop_on_tier`; (b) **`BUDGET_EXHAUSTED`** — `policy.max_iterations` reached; (c) **`PLATEAU`** — last `policy.plateau_runs` children all had `|delta_vs_parent| < policy.plateau_epsilon`; (d) **`DEAD_END`** — last `plateau_runs` hypotheses all Jaccard-similar to an earlier one above `policy.dead_end_threshold` (Model Builder is stuck rephrasing); (e) **`CONTINUE`** — keep going. On CONTINUE, the orchestrator keeps phase_4 ACTIVE, clears completed subtasks, and the next `build_lead_prompt` mints a child experiment — Model Builder auto-drafts the next `hypothesis.md` by reading parent's `result.md` shortfalls + `diagnosis.md` + `next.md` ideas, without human input. Supervised mode disables the loop (every gate pauses for human approval, as before). Policy defaults: `max_iterations=10`, `plateau_epsilon=0.01`, `plateau_runs=3`, `stop_on_tier=must_pass`, `dead_end_threshold=0.9`. Plans may override via an optional `## Experiment Loop` block.
