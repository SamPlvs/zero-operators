...
"""Filename for the human-readable running experiment log."""
...
# ---------------------------------------------------------------------------
# Human-readable experiment checklist
# ---------------------------------------------------------------------------


def render_checklist(registry: ExperimentRegistry) -> str:
    """Render a succinct, human-reviewable experiment log from the registry.

    One row per experiment in mint order: id, parent, status, hypothesis,
    primary metric, signed delta vs parent, oracle tier, and the top
    shortfall. A trailing "Next planned" section surfaces the most recent
    experiment's proposed follow-ups. Deterministic and idempotent — safe
    to regenerate after every registry mutation.

    This is the artifact a human (or the Lead) reviews to answer "what have
    we tried, how did it go, and what's next?" without parsing JSON.
    """
    header = (
        "# Experiment Checklist\n\n"
        f"Project: `{registry.project or 'unknown'}` · "
        f"{len(registry.experiments)} experiment(s) · auto-generated by ZO.\n\n"
        "_Succinct running log: what was tried, the result, and what's next. "
        "Regenerated after each experiment — do not edit by hand._\n"
    )
    if not registry.experiments:
        return header + "\n_No experiments yet._\n"

    rows = [
        "| Exp | Parent | Status | Hypothesis | Metric | Δ vs parent "
        "| Tier | Top shortfall |",
        "|-----|--------|--------|------------|--------|------------"
        "|------|---------------|",
    ]
    rows.extend(_checklist_row(e) for e in registry.experiments)
    out = header + "\n" + "\n".join(rows) + "\n"

    nxt = _next_planned(registry)
    if nxt:
        out += "\n## Next planned\n\n" + "\n".join(f"- {i}" for i in nxt) + "\n"
    return out


def write_checklist(
    registry_dir: Path, registry: ExperimentRegistry | None = None,
) -> Path:
    """Render and atomically write ``CHECKLIST.md`` into the registry dir.

    Loads the registry from disk when ``registry`` is not supplied. Called
    automatically after every registry mutation (mint / update_*) so the
    checklist always reflects current state. Returns the written path.
    """
    if registry is None:
        registry = load_registry(registry_dir)
    registry_dir.mkdir(parents=True, exist_ok=True)
    path = registry_dir / CHECKLIST_FILENAME
    tmp = registry_dir / f".{CHECKLIST_FILENAME}.tmp"
    tmp.write_text(render_checklist(registry), encoding="utf-8")
    tmp.replace(path)
    return path
...
def _checklist_row(exp: Experiment) -> str:
    """Render one experiment as a checklist table row."""
    parent = exp.parent_id or "—"
    hypothesis = _checklist_cell(exp.hypothesis)
    if exp.result is not None:
        pm = exp.result.primary_metric
        metric = f"{pm.name}={pm.value:g}"
        delta = "—" if pm.delta_vs_parent is None else f"{pm.delta_vs_parent:+g}"
        tier = exp.result.oracle_tier
        shortfall = (
            _checklist_cell(exp.result.shortfalls[0])
            if exp.result.shortfalls else "—"
        )
    else:
        metric = delta = tier = shortfall = "—"
    return (
        f"| {exp.id} | {parent} | {exp.status} | {hypothesis} | {metric} "
        f"| {delta} | {tier} | {shortfall} |"
    )


def _next_planned(registry: ExperimentRegistry) -> list[str]:
    """Return the most recent experiment's proposed follow-ups, if any."""
    for exp in sorted(
        registry.experiments, key=lambda e: e.created, reverse=True,
    ):
        if exp.next_ideas:
            return list(exp.next_ideas)
    return []
