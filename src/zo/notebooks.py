...
def _cells_training(artifacts: list[str]) -> list[nbformat.NotebookNode]:
    ...
    cells.append(new_code_cell(
        '# Load training metrics from ZO training callback JSONL\n'
        'metrics_path = PROJECT_ROOT / "logs/training/metrics.jsonl"\n'
        'train_log = []\n'
        'try:\n'
        '    with open(metrics_path) as f:\n'
        '        for line in f:\n'
        '            entry = json.loads(line.strip())\n'
        '            if entry.get("event") == "epoch_end":\n'
        '                train_log.append(entry)\n'
        '    epochs = [e["epoch"] for e in train_log]\n'
        '    train_loss = [e["metrics"].get("train_loss") for e in train_log]\n'
        '    val_loss = [e["metrics"].get("val_loss") for e in train_log]\n'
        '    print(f"Loaded {len(train_log)} epoch records")\n'
        'except (FileNotFoundError, KeyError):\n'
        '    train_log, epochs, train_loss, val_loss = [], [], [], []\n'
        '    print("Training metrics not found. Trying legacy format...")\n'
        '    # Fallback: legacy logs/training_log.json\n'
        '    legacy = PROJECT_ROOT / "logs/training_log.json"\n'
        '    try:\n'
    ...
    cells.append(new_code_cell(
        '# Learning rate schedule (from metrics JSONL)\n'
        'lr_data = [(e["epoch"], e.get("learning_rate"))\n'
        '           for e in train_log if e.get("learning_rate") is not None]\n'
        'if lr_data:\n'
        '    lr_epochs, lr_vals = zip(*lr_data)\n'
        '    fig, ax = plt.subplots(figsize=(10, 4))\n'
        '    ax.plot(lr_epochs, lr_vals)\n'
        '    ax.set_xlabel("Epoch")\n'
        ...
        '    plt.show()\n'
        'else:\n'
        '    print("LR schedule not available in training logs.")\n'
    ))
    cells.append(new_code_cell(
        '# Checkpoint history\n'
        'checkpoints = []\n'
        ...
        '        for line in f:\n'
        '            entry = json.loads(line.strip())\n'
        '            if entry.get("event") == "checkpoint_saved":\n'
        ...
        '        ep = ckpt.get("epoch", "?")\n'
        '        path = ckpt.get("checkpoint_path", "")\n'
        '        metrics = ckpt.get("metrics", {})\n'
        '        m_str = ", ".join(f"{k}={v:.4f}" for k, v in metrics.items())\n'
        '        print(f"  Epoch {ep}: {m_str}  [{path}]")\n'
        'else:\n'
