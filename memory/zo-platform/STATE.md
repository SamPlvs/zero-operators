...
**Session 024 hand-off — pick up here.** PR #57 merged (`--low-token` mode core). PR #58 OPEN + MERGEABLE on `claude/docs-fix-team-404-and-mdx-dollar` containing 3 commits: (1) MDX `$` rendering fix in prose (escaped 9 occurrences across `low-token-mode.mdx` / `quickstart.mdx` / `cost-benchmark.mdx`); (2) sub-agent model override in lead prompt — `_prompt_low_token_overrides()` instructs lead to pass `model="claude-sonnet-4-6"` to every `Agent()` call when low-token is on (addresses Claude Code 2.1.92's TeamCreate-spawns-Opus-regardless-of-frontmatter behaviour); (3) the-team page 404 root cause fix — line 51 had prose `<500 lines, functions <50 lines` which MDX 2 parser treats as malformed JSX → page failed to compile → Mintlify excluded from build. Also: legacy `/concepts/agents` → `/concepts/the-team` 301 redirect rule added; `docs.zero-operators.dev` URL corrected to actual deploy `docs.zerooperators.com` across README badge / docs/README / STATE.md.

**Next action when picking up:** merge PR #58, then run the mini-bench manually to verify whether Claude Code's `Agent()` tool honours the `model` parameter. The harness is at `scripts/benchmark_low_token.sh`. The mini-test variant: launch `zo build /tmp/zo-bench-low/.zo/plans/mnist-bench-low.md --low-token --gate-mode full-auto` from inside tmux, wait ~3-4 min until first sub-agent spawn, run `ps aux | grep "agent-name" | grep "model"` — if `claude-sonnet-4-6` the override works (full ~70-80% savings achievable), if `claude-opus-4-6` Claude Code ignores the param (SDK refactor needed; lead-side savings still apply). Cost ~\$3-5, time ~5 min, decisive yes/no. Previous full-bench attempt (2026-04-26) stalled at \$13.59/17min on a separate Claude-Code-vs-team-permissions issue (lead idle, didn't actively process sub-agent permission requests) — that's documented in `docs/reference/cost-benchmark.mdx` "Findings from the partial run" but is OUT OF SCOPE for low-token; affects all autonomous builds equally.
