...
**Session 025 in-progress — pick up here.** PR #58 merged (commit 45b2fdc). First `--low-token` MNIST bench completed end-to-end: lead + 3 sub-agents (data-engineer, code-reviewer, test-engineer) all confirmed running on `claude-sonnet-4-6` via `ps aux` — **the override mechanism in `_prompt_low_token_overrides()` ships as designed.** Claude Code 2.1.107 honours `model="claude-sonnet-4-6"` on `Agent()` spawns. SDK refactor previously flagged in STATE no longer needed for the override piece. **BUT** the bench surfaced two material findings:

1. **Cost claim was overstated.** Phase 1 alone burned $3.47 (lead $1.59 + sub-agents $1.88 on Sonnet, mid-run snapshot via `npx ccusage --instances`). Final number TBD — bench finished, user is to capture totals via `npx -y ccusage --instances --since "$(date -u +%Y%m%d)"`. Projected final $7-10 vs. ~$11 default = **~10-35% savings, not the 70-80% claimed** in `cost-benchmark.mdx`, `low-token-mode.mdx`, README. Root cause: in default mode sub-agents are *already* on Sonnet via their `.md` frontmatter; only the lead is Opus. So `--low-token` swaps lead Opus→Sonnet (~5× cheaper for ~30-40% of total cost) ≈ 25-30% savings ceiling. The "70-80%" assumed everyone was on Opus. They never were. **Doc cascade pending:** `cost-benchmark.mdx`, `low-token-mode.mdx`, `low-token-preset.mdx`, README, STATE all need honest measured numbers + a "what doesn't help and why" section + a "what would help" section (prompt caching via SDK refactor for ~50-60% on cache portion, Batch API 50% off, Files API for repeated context). User has agreed to the cascade — waiting on final ccusage number from completed bench.

2. **`zo watch-training` contract violation surfaced + FIXED.** Training completed (98.83% test acc) but `zo watch-training` rendered "Waiting for training to start..." for the entire run. Investigation revealed three stacked contract violations: (a) `model-builder.md` had two contradictory paths — Phase 4 section said `.zo/experiments/<exp_id>/` via `for_experiment()`, "Training Metrics Protocol REQUIRED" section said `log_dir="logs/training"`. (b) Consumers (`wrapper.py:530` auto-split, `cli.py:2638` watch-training) hardcoded `<delivery>/logs/training/` — wrong path even if the agent had complied with Phase 4. (c) The Phase 4 gate only checked `result.md`, not the actual `ZOTrainingCallback` artifacts. Sonnet (low-token) ignored both contradicting instructions, wrote vanilla PyTorch with manual JSON dump to `experiments/exp-001/results.json` (a third path the agent invented). Gate passed because the gate was aspirational, not enforced. **Fix shipped this session:** new `resolve_active_experiment_dir()` helper in `src/zo/experiments.py` (canonical path resolver: most-recent RUNNING > most-recent COMPLETE > None); both consumers refactored to use it; orchestrator's `_finalize_experiments` extended to require `metrics.jsonl` + `training_status.json` + `result.md` (gate fails with rationale "ZOTrainingCallback not used" when Model Builder bypasses the callback); `model-builder.md` contradiction removed and reconciled to single source of truth; `specs/agents.md` Section 3 updated to match. +19 tests (706 → 725 + 7 skipped). PR-035 captures the lesson: aspirational agent contracts get ignored under sub-optimal models — hard gate enforcement is mandatory.

**Next action when picking up:** (1) capture final bench cost (`npx -y ccusage --instances --since "$(date -u +%Y%m%d)"`) and update `cost-benchmark.mdx` with measured numbers; (2) cascade-update `low-token-mode.mdx`, `low-token-preset.mdx`, README, STATE with honest savings figure; (3) commit + PR for the watch-training fix shipped this session (changed: `experiments.py`, `cli.py`, `wrapper.py`, `orchestrator.py`, `.claude/agents/model-builder.md`, `specs/agents.md`, 5 test files, PRIORS.md, DECISION_LOG.md). Both pieces can ship as separate PRs or one bundled — user to decide.
...
benchmark: scripts/benchmark_low_token.sh harness in place; first measured attempt 2026-04-26 aborted at ~17min/$13.59 — lead idle on sub-agent permission gate; surfaced architectural finding that Claude Code 2.1.92 spawns TeamCreate sub-agents on `claude-opus-4-6` regardless of agent .md frontmatter `model:` field. **FIX SHIPPED:** orchestrator's `_prompt_low_token_overrides()` now adds a "Low-Token Sub-Agent Model Override" section to the lead prompt instructing the Lead Orchestrator to pass explicit `model="claude-sonnet-4-6"` to every `Agent()` spawn. If `Agent()` honours the parameter, full ~70-80% savings achievable; if not, graceful fallback (Lead logs a DECISION_LOG entry, sub-agents stay on Opus, lead-side savings still apply). Second bench attempt needed to confirm. +2 unit tests covering override section presence/absence by mode.
demo_results:
