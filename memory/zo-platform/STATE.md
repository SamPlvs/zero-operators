...
10. **Low-resources / serial-spawn mode** — investigate adding a `--low-resources` CLI flag (or plan-level `## Resources` block toggle) that tells the Lead Orchestrator to spawn agents **serially within a phase** instead of in parallel. Motivation: `classical_ml` Phase 1 spawns 5 agents in parallel (`data-engineer` + `test-engineer` + `research-scout` + `code-reviewer` + `domain-evaluator` — see [_orchestrator_phases.py:61-64](src/zo/_orchestrator_phases.py)) which is the heaviest spawn burst in any default workflow. On Macs near macOS's `kern.maxprocperuid=2666` cap, this burst trips `fork()` failures and crashes Claude Code sessions (per session-022 diagnosis + PR #52 troubleshooting doc). PR-015 deliberately set 5-agent Phase 1 as the production-ready default, so a flag-gated alternative — not a default change — preserves PR-015 while giving resource-constrained users an escape hatch. Roughly halves wall-clock per phase (serial vs parallel) but removes the burst spike entirely. Touch points: lead prompt's `_prompt_coordination()` (instruct serial vs parallel), CLI flag plumbing, optional plan-level override. Owner: Sam, picking up next session. **Cross-ref:** PR #52 troubleshooting doc, PR-015 prior, session-022 diagnosis.
