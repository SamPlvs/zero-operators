...
## Decision: 2026-04-26T18:00:00Z
**Type:** ARCHITECTURE
**Title:** Documentation site framework — Mintlify under `docs/`, public at `docs.zero-operators.dev`
**Decision:** Adopted Mintlify as the docs-site framework. Scaffolded under `docs/` (alongside existing `docs/*.md` files which stay in place during transition). New layout: `docs/mint.json` (config — nav, brand colours #D87A57 primary on canvas/paper backgrounds, Geist font), `docs/{logo/light.svg, logo/dark.svg, favicon.svg}` (simplified C mark + Geist wordmark), and 13 MDX content pages: introduction, quickstart, installation, concepts/{overview, the-plan, the-oracle, agents, phases-and-gates, memory-and-continuity}, cli/{overview, init, draft, build}. `docs/README.md` documents local dev (`mintlify dev`), file structure, brand tokens, deploy steps, and the migration table from existing `.md` sources to MDX destinations.
**Rationale:** User asked for documentation comparable to Paperclip / OpenWebUI sites. Initial framework debate weighed Mintlify against Astro Starlight; the deciding frame was *user adoption*, not brand fidelity. Mintlify wins on adoption because (a) pattern recognition — Anthropic, Vercel, Linear, Cursor, Resend all use Mintlify, so devs land knowing where everything is; (b) AI search is built-in and increasingly table stakes for dev tools; (c) positioning signal — same visual language as the Anthropic API docs auto-categorises ZO as a "serious project"; (d) battle-tested mobile + edge-cases across thousands of customer sites; (e) lower maintenance burden = fresher docs over time. Brand fidelity matters most on the marketing site (`website/`), where the user decides if ZO is for them — once they're in docs, they're solving a problem and want speed-to-answer, not typography continuity.
**Alternatives considered:** (1) Astro Starlight — full brand control + same Astro stack as `website/`, but no AI search, less battle-tested edge cases, more maintenance. Rejected after re-framing on user adoption. (2) Docusaurus (Meta) — what OpenWebUI uses; React-based, Apache-2; powerful but lacks Mintlify's docs-specific polish. (3) MkDocs Material — fits Python ecosystem but visually less modern. (4) GitBook — hosted SaaS, free tier exists but adoption signal weaker. (5) Just `/docs` markdown in repo — zero lift, zero polish; ruled out because user wants Paperclip-quality. Chose Mintlify for the highest UX/effort ratio + strongest adoption signal.
**Outcome:** First content slice shipped: 13 pages covering Get-Started + Concepts + 4 key CLI commands. Existing `docs/COMMANDS.md`, `docs/SAMPLE_PROJECT.md`, `docs/TROUBLESHOOTING.md`, `docs/DELIVERY_STRUCTURE.md` left in place during transition (README still links to them); migration table in `docs/README.md` lists pending follow-up pages. Logo + favicon SVGs use the simplified C mark from brand v2 with paper/canvas-aware variants. Brand colours pinned in `mint.json`: primary `#D87A57` (coral), light bg `#F4EFE6` (paper), dark bg `#12110F` (canvas). Custom domain `docs.zero-operators.dev` planned (free on Mintlify OSS plan); deploy is one-click after Mintlify connects to the GitHub repo. Remaining migration work (CLI pages for remaining commands, tutorials/{mnist,cifar10}, reference/{plan-schema, agents, slash-commands, workflow-modes}, architecture/*, resources/{troubleshooting, changelog, priors, contributing}) tracked as follow-up.
...
## Decision: 2026-04-26T20:00:00Z
**Type:** ARCHITECTURE
**Title:** `--low-token` mode shipped — preset profile for Pro plan / student / budget-constrained users
**Decision:** Added a single CLI flag `--low-token` and matching plan YAML field `low_token: true` that activate a cost-saving preset across multiple existing knobs in unison: (1) lead model swap Opus → Sonnet (~5× cheaper input/output), (2) Phase-4 `max_iterations` clamp 10 → 2, (3) `stop_on_tier` clamp `must_pass` → `could_pass`, (4) drop `research-scout` from cross-cutting agents (kept `code-reviewer` — silent quality drift is worse than the saved tokens), (5) disable the Haiku headline ticker (~60 small calls/hr), (6) default gate-mode to `full-auto` (no human-loop overhead), (7) set `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE=60` in the Claude Code subprocess env for earlier auto-compaction. Override flags `--lead-model {opus,sonnet,haiku}`, `--max-iterations N`, `--no-headlines` compose with the preset. Precedence is CLI flag > plan field > preset default > base default — so a plan with `low_token: true` AND a `## Experiment Loop` block specifying `max_iterations: 8` runs at 8 iterations (plan wins over clamp). Banner gets a `[low-token]` badge for visual confirmation. Estimated savings ~70-80% (MNIST-equivalent runs drop from ~$11 to ~$2-3). Implementation: `_LOW_TOKEN_PRESET` constant in `cli.py:255` is the single source of truth; `_resolve_lead_model` and `_resolve_gate_mode` helpers encode precedence; `LoopPolicy.low_token` field + `_LOW_TOKEN_LOOP_CLAMPS` dict + `resolve_policy(spec, *, low_token, max_iterations_override)` keyword-only kwargs; `Orchestrator.__init__(low_token, max_iterations_override)` threads through `_agents_for_phase` (filters `research-scout`) and `build_lead_prompt` (drops dedicated `_prompt_adaptations` section + emits compact roster); `LifecycleWrapper.launch_lead_session(extra_env=...)` propagates env vars (tmux: prefix `KEY=VAL` to the shell command; headless: `subprocess.Popen(env=...)`). New `docs/concepts/low-token-mode.mdx` covers preset table, trade-offs, when-not-to-use; `mint.json` nav extended; `cli/build.mdx` Options table + Examples accordion updated. README gains "Built on" acknowledgements section listing Claude Code + 11 deps with licenses, plus "Optional integrations (planned)" listing ccusage / Repomix / caveman as future work. Tests: +29 — `TestResolvePolicyLowToken` (5), `TestAgentsForPhaseLowToken` (3), `TestLowTokenOrchestrator` (4), `TestLowTokenFlags` (8 incl. preset-shape, banner badge, helper precedence), `TestLowTokenPropagation` integration (5), `TestFrontmatter` low_token/lead_model parsing (3), and a couple in adjacent test classes. 704 passed + 7 skipped, ruff clean on `src/zo/`, validate-docs 10/10 (1 pre-existing test-count warning).
**Rationale:** A friend ran ZO end-to-end and pushed back on token spend — MNIST cost ~$11 (~$9.50 was Opus lead prompts × 10 Phase-4 iterations). Pro-plan subscribers (and student accounts) hit daily caps fast and were effectively locked out. Pre-design ecosystem survey looked at three plausible external tools (ccusage for monitoring, Repomix for repo packing, caveman for terse output) — all MIT, all CLI-shaped (compatible with ZO's launcher pattern), but each needed real-world validation before bundling. Phase A scope (this PR) is the *internal* preset; Phase B/C (external integrations + `zo usage` ccusage wrapper) are deferred to follow-up PRs after the preset proves itself in real Pro-plan use. The architectural constraint that ruled out prompt caching / Batch API / Files API / extended thinking budget tuning is that ZO uses `claude` CLI subprocess (no Anthropic SDK direct usage) — those features all require SDK access and would be a substantial refactor. The single-profile design (one flag, not three tiers `lite`/`standard`/`pro`) is deliberate v1 simplicity; tiers if real users ask for in-between settings.
**Alternatives considered:** (1) Three-tier presets (`lite`/`standard`/`pro`) — premature surface area without value; defer to v2 if real users complain. (2) Bundle ccusage / Repomix / caveman — caveman is 3 weeks old (production risk), all need real-world validation against ZO flows; better to ship the internal preset first and acknowledge the externals as planned. (3) Refactor to direct Anthropic SDK to enable prompt caching — would unlock 90% caching savings on input but is a substantial refactor that breaks the tmux interactive UX (requires reimplementing TeamCreate, Agent peer-to-peer, MCP, etc.). Out of scope. (4) Lead model = Haiku instead of Sonnet — Haiku 4.5 is excellent on coding but multi-step orchestration / contract reasoning is its weak suit; Sonnet is the safer trade. (5) Drop `code-reviewer` too from cross-cutting in low_token — silent quality drift is a worse failure mode than the ~6 saved spawns × ~1KB; kept it on. (6) Skip `_prompt_adaptations` AND inline adaptations — no, inline adaptations stay because they're per-spawned-agent context, lose them and the agent doesn't know domain priors. Just drop the duplicated dedicated section.
**Outcome:** Branch `claude/lucid-herschel-fece72`. Files modified: `src/zo/{plan,experiment_loop,orchestrator,wrapper,cli}.py`, `tests/unit/{test_plan,test_experiment_loop,test_orchestrator,test_dynamic_agents,test_cli}.py`, `tests/integration/test_low_token.py` (new), `README.md`, `docs/cli/build.mdx`, `docs/concepts/low-token-mode.mdx` (new), `docs/mint.json`, `docs/COMMANDS.md`, `memory/zo-platform/{STATE,DECISION_LOG}.md`. No PR-XXX prior added to PRIORS.md — this was a feature design, not a failure-driven self-evolution; design rationale captured in this DECISION_LOG entry is the auditable record.

---

## Decision: 2026-04-26T21:30:00Z
**Type:** BUGFIX
**Title:** docs/concepts/the-team.mdx frontmatter title fix + README test-count badge sync
**Decision:** Fixed two stale-doc inconsistencies surfaced when the user reported "the team section returns 404": (1) `docs/concepts/the-team.mdx` frontmatter `title:` was `"Agents"` (legacy from the pre-rename era, before commit a5a0592 renamed `concepts/agents` → `concepts/the-team`). The file path, mint.json nav entry, and every internal link in other MDX files (`introduction.mdx`, `cli/init.mdx`, etc.) all use `/concepts/the-team` — only the title was stale. Updated to `"The team"` matching the convention used by sibling pages (`"The plan"`, `"The oracle"`). (2) `README.md` test badge `tests-675_passing` was the count from before this session's +29 low-token tests; updated to `tests-704_passing` matching the actual `pytest -q` output. Verified no other stale `concepts/agents` references exist anywhere in the repo via `grep -rn "concepts/agents"` (confirmed empty). All other doc page titles audited — every other concept/cli/reference page has frontmatter title matching the file path semantics.
**Rationale:** The user's reported 404 came from cognitive mismatch + Mintlify deployment lag: the sidebar (driven by frontmatter title) said "Agents" while the navigation link text in introduction.mdx said "The team" pointing to `/concepts/the-team`. On a Mintlify deploy that had cached the old `agents.mdx` filename (pre-rename), the slug `/concepts/agents` would 404 even though `the-team.mdx` was the source of truth. Aligning the title to "The team" eliminates the surface confusion; future Mintlify deploys will pick up the correct content with consistent labeling. The README badge fix is hygiene — PR-005 principle (aspirational rules without enforcement degrade): the badge should track the actual test count or it loses signal value. validate-docs Check 6 still issues a warning because it counts grep-matched function definitions (692, due to pytest parameterization expanding to 704 at runtime) — that's a measurement-method discrepancy, pre-existing, documented previously.
**Outcome:** 2 file edits (`docs/concepts/the-team.mdx` line 2, `README.md` line 13). 704 tests pass + 7 skipped. ruff `src/zo/` clean. validate-docs 10/10 with the same 1 pre-existing test-count warning. STATE.md cascaded with both fixes.

---

## Decision: 2026-04-26T22:30:00Z
**Type:** BUGFIX
**Title:** Mintlify low-token-mode page font/spacing inconsistency — unescaped MDX `$` parsed as LaTeX math delimiters
**Decision:** Escaped 9 occurrences of `$` in prose across `docs/concepts/low-token-mode.mdx` (2), `docs/quickstart.mdx` (1), and `docs/reference/cost-benchmark.mdx` (6) by replacing `$N` with `\$N`. Code-block `$` left intact (fenced code is not MDX-parsed). Verified with regex sweep that all remaining unescaped `$` in tracked `.mdx` files are inside ` ```bash ` or ` ` ``inline` ` ` code regions.
**Rationale:** User screenshot showed garbled italic prose: "The MNIST end-to-end demo cost ~11ondefaultsettings.With`--low-token`thesamerunlandsat**2-3**". Mintlify's MDX 2 parser treats unescaped `$...$` as LaTeX math-mode delimiters, so the pair `$11 on default settings. With...the same run lands at $2-3` was being rendered as math content with text typeset and spaces stripped. This is a Mintlify-specific parser quirk that doesn't affect rendering on GitHub or other markdown viewers — surfaced only when the page hit the deployed Mintlify build.
**Outcome:** PR #58 commit `b453902`. Deploys cleanly on next Mintlify rebuild.

---

## Decision: 2026-04-26T22:45:00Z
**Type:** ARCHITECTURE
**Title:** Sub-agent model override injected into lead prompt for low-token mode
**Decision:** Added `_prompt_low_token_overrides()` to `Orchestrator` (in `src/zo/orchestrator.py`). Returns empty string when `self._low_token` is False; when True, returns a "Low-Token Sub-Agent Model Override" section that's joined into `build_lead_prompt()` between coordination and gate-criteria sections. The section instructs the Lead Orchestrator to pass an explicit `model="claude-sonnet-4-6"` parameter to every `Agent()` spawn — bypassing Claude Code's default of Opus for `TeamCreate`-spawned sub-agents. Includes a graceful fallback clause: if the runtime `Agent()` tool ignores the parameter, the lead logs a `DECISION_LOG` entry and proceeds; lead-side savings still apply. Two new orchestrator tests verify the section is present when `low_token=True` and absent in default mode.
**Rationale:** Empirical finding from the partial bench attempt 2026-04-26 (~17min/$13.59 aborted run): Claude Code 2.1.92's `TeamCreate` spawns sub-agents with `--model claude-opus-4-6` regardless of what the agent's `.md` file declares. `data-engineer.md` declared `model: claude-sonnet-4-6` but the spawn process used Opus. Without an override, low-token mode only saved on the lead's own session — sub-agents (data-engineer, model-builder, oracle-qa, code-reviewer, test-engineer) stayed on Opus, so the workhorses of any actual ML run were unaffected. The prompt-level fix is zero-cost (telling lead to try; falls back gracefully) and tests the actual question: does Claude Code's `Agent()` tool accept a `model=` parameter from the lead's tool-use call? Pending mini-bench confirmation.
**Alternatives considered:** (1) Modify `.claude/agents/*.md` frontmatter at build time — race-prone on concurrent builds, messy backups. (2) Use Claude Code's CLI `--dangerously-skip-permissions` to bypass approval — incompatible with tmux interactive launch (PR-001). (3) Switch ZO to direct Anthropic SDK so sub-agent models are programmatically controlled — substantial refactor, breaks tmux UX, already documented as out-of-scope. The prompt-level instruction is the minimum viable fix that doesn't compromise architecture.
**Outcome:** PR #58 commit `1e5fad1`. Test count 704 → 706 (+2). docs/concepts/low-token-mode.mdx FAQ adds "Do sub-agents (data-engineer, model-builder, etc.) also run on Sonnet in low-token mode?" Q&A. docs/reference/cost-benchmark.mdx restructured "Implications" section into "Fix shipped (post-bench)" with the new override + caveat. STATE.md `benchmark` line updated to record FIX SHIPPED + revised savings estimate (50-65% pending mini-bench confirmation; 70-80% if Agent() honours the parameter).

---

## Decision: 2026-04-27T07:25:00Z
**Type:** BUGFIX
**Title:** docs.zerooperators.com `/concepts/the-team` 404 root cause — MDX 2 parser killed the page on `<500 lines` prose
**Decision:** Replaced `files <500 lines, functions <50 lines` with `files under 500 lines, functions under 50 lines` on line 51 of `docs/concepts/the-team.mdx`. The replacement preserves the original semantic meaning (file/function size limits from CLAUDE.md coding conventions) while avoiding the MDX 2 parser bug. Repo-wide regex sweep with code-block exclusion confirmed this was the only such pattern in any tracked `.mdx` file.
**Rationale:** Multiple users (Sam + friends) reported persistent 404 on `https://docs.zerooperators.com/concepts/the-team` despite the file existing in source, mint.json registering it, and frontmatter title being correct. Direct HTTP probe of the deployed Mintlify site proved sibling concept pages return 200 (e.g. `/concepts/the-plan`) — so the build was current but the team page specifically was excluded from the build output. Inspection of the page MDX revealed line 51's `<500 lines` and `<50 lines` in prose. **MDX 2 (used by Mintlify) treats `<` followed by any non-whitespace character as the start of a JSX opening tag — including digits.** So `<500` was parsed as a malformed JSX attempt, the file failed compilation, Mintlify silently dropped it from the build → 404. Every prior attempted fix (rename agents → the-team, frontmatter title sync, redirect rule) was operating on infrastructure that Mintlify never even processed. The redirect rule from PR #58 commit `b453902` (`/concepts/agents` → `/concepts/the-team`) is still useful for stale bookmarks/caches but was orthogonal to this root cause.
**Bonus fix in same commit:** Stale `docs.zero-operators.dev` URL (originally-planned domain) corrected to actual deployed `docs.zerooperators.com` (no hyphen) across `README.md` docs badge, `docs/README.md` (2 occurrences), and `STATE.md docs_site` line. DECISION_LOG entry for original Mintlify framework decision (`2026-04-26T18:00:00Z`) left intact as historical audit; STATE.md now records both planning URL and actual deploy URL with explanatory note.
**Outcome:** PR #58 commit `10c492c`. After PR merges + Mintlify auto-redeploys, `https://docs.zerooperators.com/concepts/the-team` should return 200 (currently 404). Verification command: `curl -sI https://docs.zerooperators.com/concepts/the-team | head -1` post-deploy. 706 tests pass + 7 skipped. ruff src/zo/ clean. validate-docs 10/10. New PRIOR-worthy lesson: MDX 2 prose containing `<` immediately followed by a non-space character is a silent compilation killer — should be added to a future MDX-authoring guidelines doc and possibly enforced via a pre-commit lint rule.

---

## Decision: 2026-04-27T15:30:00Z
**Type:** BUGFIX + EVOLUTION
**Title:** `zo watch-training` rendered "Waiting" forever — Phase 4 gate now hard-enforces `ZOTrainingCallback` artifacts; canonical path resolver added
**Decision:** Three-part fix.
1. **New helper `resolve_active_experiment_dir(delivery_repo)`** in `src/zo/experiments.py` — single source of truth for "where is the active experiment writing?". Resolution order: most-recent RUNNING > most-recent COMPLETE > None. Skips FAILED and ABORTED.
2. **Consumers refactored** to use the helper: `cli.py:watch_training` and `wrapper.py:_maybe_open_training_pane()`. Both previously hardcoded `<delivery>/logs/training/training_status.json` — never matched what `ZOTrainingCallback.for_experiment()` actually writes (under `.zo/experiments/<exp_id>/`). The wrapper also now passes `--repo` to the spawned `zo watch-training` subprocess for cwd-independent resolution.
3. **Gate hardened**: `orchestrator._finalize_experiments()` now requires `metrics.jsonl` AND `training_status.json` AND `result.md` in every running experiment's dir. Missing artifacts surface in the gate rationale with the literal hint "ZOTrainingCallback not used", forcing the next iteration's Lead prompt to make the root cause unambiguous to the Model Builder.
4. **Contract reconciled**: `.claude/agents/model-builder.md` had two contradictory instructions for where to write metrics — the agent's Phase 4 section pointed at `.zo/experiments/<exp_id>/` via `for_experiment()`, but the prominent "Training Metrics Protocol REQUIRED" code example used `log_dir="logs/training"`. Stale section removed; the Phase 4 path is now the single source of truth in the agent's persistent contract. `specs/agents.md` Model Builder entry updated to match.

**Rationale:** First `--low-token` MNIST bench (session 025) finished training successfully (98.83% test acc) but `zo watch-training` showed "Waiting for training to start..." for the entire run. The Model Builder (Sonnet, low-token mode) wrote vanilla PyTorch with a manual JSON dump to `experiments/exp-001/results.json` — neither the `.zo/experiments/` path nor the `logs/training/` path. The Phase 4 gate STILL passed because it only checked `result.md` existence. Three contract violations stacked: (a) the agent contract was internally contradictory; (b) consumers (watch-training, auto-split-pane) looked at the wrong path even if the agent had complied; (c) the gate was aspirational — it didn't actually verify the artifacts existed. PR-005 ("enforcement > aspiration") principle applies: documentation alone isn't a contract; the orchestrator gate is.

**Alternatives considered:**
- Make watch-training fall back to `logs/training/` if `.zo/experiments/` is empty. Rejected — entrenches the legacy path and lets agents continue bypassing the capture layer.
- Auto-detect the agent's chosen path and update the dashboard. Rejected — pretends the contract is flexible. The autonomous loop, `zo experiments`, and Phase 4 notebooks all assume `.zo/experiments/`. Auto-detect would mean four divergent path resolvers across the codebase.
- Generate a stub `train.py` template at scaffold time. Considered, but deferred — the fix already eliminates the ambiguity at contract level + gate level. Template is a v1.1 nice-to-have.

**Outcome:** New `resolve_active_experiment_dir()` in `src/zo/experiments.py` (40 lines including docstring); `cli.py:watch_training` and `wrapper.py:_maybe_open_training_pane` refactored to consume it; `orchestrator._finalize_experiments` extended to require `metrics.jsonl` + `training_status.json`; `model-builder.md` contradiction removed; `specs/agents.md` Section 3 updated to match. **Tests:** +19 (706 → 725, +7 skipped) — 8 unit (`TestResolveActiveExperimentDir`), 4 integration (`TestPhase4GateRequiresTrainingArtifacts`), 3 unit (`TestWatchTrainingPathResolution`), 4 unit (`TestMaybeOpenTrainingPane`). Plus `test_experiment_flow.py` + `test_auto_iteration.py` helpers updated to write the new required artifacts (no test logic changes, just helper expansion). ruff `src/zo/` clean. validate-docs 10/10 (1 pre-existing test-count warning, unrelated). Captured in PRIORS.md as PR-035. PR #59 merged 2026-04-27 (commit f290311).

---

## Decision: 2026-04-27T16:30:00Z
**Type:** DOCUMENTATION + EVOLUTION
**Title:** Cost-benchmark honesty cascade — replace 70-80% projection with measured ~30% across docs/README/STATE
**Decision:** Replace the unmeasured 70-80% / ~\$2-3 savings claim with the measured **\$7.75 (~30% reduction)** number across all surfaces, and add structural reasoning ("why 30%, not 70-80%") plus a roadmap to higher savings ("what would push this higher").

Cascade scope:
- **`docs/reference/cost-benchmark.mdx`** — Note callout updated; "Estimated results (pre-measurement)" → "Measured results (2026-04-27)"; new "Why the savings ceiling is structural" section explaining the role-by-role token-share math (lead 30-40%, sub-agents 60-70% on Sonnet already); new "What `--low-token` actually moves" knob-by-knob attribution table; new "What would push savings higher" roadmap (Haiku right-sizing, Phase-1 trim, prompt caching via SDK refactor); "Findings from the partial run (2026-04-26)" rewritten to "Findings from the first measured run (2026-04-27)" capturing the override-mechanism confirmation + the watch-training contract violation discovery; final tracking table now has the 2026-04-27 row populated.
- **`docs/concepts/low-token-mode.mdx`** — "Estimated savings" → "Measured savings" with honest 30% number + plan-shape caveats (full-lifecycle plans ~30%, no-Phase-4 plans ~15-20%); "Worked example: MNIST" updated to show measured cost breakdown (Phase 1 alone \$3.47, full bench \$7.75); FAQ entry on sub-agents-on-Sonnet rewritten — they were always Sonnet via `.md` frontmatter, the override is defence-in-depth not a fix.
- **`docs/reference/low-token-preset.mdx`** — added Note callout at top with measured \$7.75 / 30% headline + link to cost-benchmark for the breakdown.
- **`README.md`** — `--low-token` paragraph rewritten: "MNIST-equivalent runs go from ~\$11 to ~\$2-3" → "Measured on the MNIST bench (2026-04-27): ~30% reduction (\$7.75 vs. ~\$11 default)"; structural-ceiling caveat added; link to cost-benchmark for the path to higher savings.
- **`memory/zo-platform/STATE.md`** — session 025 hand-off rewritten with measured number; the line-21 status block's "70-80%" claim replaced with "~70-80% pre-measurement (now refuted — measured ~30%)"; benchmark line at the bottom rewritten with measured numbers + path-to-higher roadmap.

**Rationale:** PR-005 ("enforcement > aspiration") principle applied to marketing claims, not just code. The 70-80% projection in v1.0.2 announcements was unbacked — it extrapolated the per-token rate (5× cheaper Sonnet vs Opus) to the whole run, but the whole run was already mostly Sonnet via sub-agent `.md` frontmatter. Once we measured the actual run, the structural ceiling became obvious: only the lead's ~30-40% cost share is affected by the swap, ceiling ~25-30%. Rather than try to spin the 30% as bigger than it is, document it honestly + lay out the architectural levers needed to push higher (Haiku right-sizing of pattern-matching agents, prompt caching, Batch API, Files API).

**Alternatives considered:**
- Keep the 70-80% claim "as a target" with a footnote. Rejected — that's bait-and-switch. Users on Pro/student plans need accurate numbers to plan around their daily message budget.
- Re-bench in default mode to confirm ~\$11 is still accurate. Considered, but ~\$11 is the historical anchor from session-005 MNIST run; re-measurement would cost another ~\$11 and add maybe ±\$2 of precision — not worth it for the claim.
- Wait until the SDK refactor lands and re-bench then. Rejected — SDK refactor is multi-week, and shipping honest interim numbers now is more important than waiting for the better number.
...
## Decision: 2026-05-01T00:00:00Z
**Type:** BUGFIX + EVOLUTION
**Title:** STATE.md phase-status validation moved to parse time — clear error replaces cryptic enum-coerce crash
**Decision:** Added `_VALID_PHASE_STATUSES` frozenset constant in `src/zo/_memory_formats.py` mirroring the `PhaseStatus` enum. `parse_state` now validates each line of the `## Phases` section against this set as it parses; an unknown status raises `ValueError(f"Invalid phase status {status!r} for {pid!r} in STATE.md ## Phases section. Valid statuses: {valid}. Hand-edited STATE.md must use only these values; see PhaseStatus enum in zo._orchestrator_models.")`. Three new tests in `tests/unit/test_memory.py`: (1) drift guard asserting `{s.value for s in PhaseStatus} == _VALID_PHASE_STATUSES`, (2) unknown-status rejection with assertions on the error message naming the offending phase, status, file, and valid options, (3) all six valid statuses round-trip. Test count 735 → 738 + 7 skipped. Orchestrator side (`_restore_phase_states`) intentionally untouched — moving validation up to the parser is the right contract; defense-in-depth at the orchestrator would mask future malformed STATE.md without surfacing the file-level cause.

**Rationale:** A real `zo continue` run on prod-001 crashed with `ValueError: 'prep_complete' is not a valid PhaseStatus` deep inside `Orchestrator._restore_phase_states` at line 318. The stack trace pointed at `enum.py` and the orchestrator, with no hint that the cause was a hand-edited STATE.md file. Root cause: an operator hand-edited the project STATE.md to express the mental model "phase 3 has a prep wave and a main wave" by inventing the status `prep_complete` and the phase id `phase_3_main`. The `phase_3_main:` line was silently ignored by `_PHASE_LINE_RE` (regex requires `phase_<digits>:` exactly), but `phase_3: prep_complete` matched the regex and `prep_complete` flowed into `phase_states["phase_3"]` as a raw string. The orchestrator then coerced it via `PhaseStatus(value)` and crashed. STATE.md is human-readable and hand-editable by design (CLAUDE.md "On Every Commit" instructs both Claude and humans to update it directly); a memory layer with that contract should fail fast at load time with a clear error, not deep in a downstream consumer.

**Alternatives considered:** (1) Make `phase_states` field type `dict[str, PhaseStatus]` in `_memory_models.py` so pydantic does the validation. Rejected — couples the memory model to orchestrator types, and `_memory_models.py` is intentionally lower in the import graph than `_orchestrator_models.py`. The string-set + drift-guard test gets the same correctness without the coupling. (2) Catch the ValueError in `_restore_phase_states` and skip unknown values with a warning. Rejected — silent skip would mask the data-quality issue and make the next session start from `pending` for a phase the operator thought was further along. Loud failure with a fix-it-yourself message is the right ergonomic. (3) Allow unknown statuses with a warning. Rejected — same as (2). The status field is a closed enum; there is no "extension" use case.

**Outcome:** Branch `claude/state-md-status-validation`. Files modified: `src/zo/_memory_formats.py` (+12 lines: constant + validation block), `tests/unit/test_memory.py` (+62 lines: 3 tests in new `TestPhaseStatusValidation` class), `memory/zo-platform/{STATE,DECISION_LOG,PRIORS}.md`, `memory/zo-platform/sessions/session-027-2026-05-01.md`. Test count 735 → 738 + 7 skipped. ruff `src/zo/_memory_formats.py` clean. validate-docs.sh 9 passed / 0 failed / 2 warnings (test-count badge stale + blocklist file missing — both pre-existing). PR-036 added to PRIORS.md.


---

## Decision: 2026-05-01T01:00:00Z
**Type:** BUGFIX + EVOLUTION
**Title:** `get_current_phase` returns ACTIVE phases on resume — interrupted sessions no longer drop in-flight work
**Decision:** Extended `Orchestrator.get_current_phase` in `src/zo/orchestrator.py` with an ACTIVE-resolution branch placed between the existing GATED-first and PENDING-with-deps-met branches. Resolution order is now (1) GATED — needs human decision, (2) ACTIVE — resume in-flight phase from prior session, (3) PENDING with all dependencies COMPLETED — fresh start. BLOCKED phases remain intentionally skipped (require human escalation outside the normal resume path). Five new tests added to `TestGetCurrentPhase` in `tests/unit/test_orchestrator.py`: `test_returns_active_phase_on_resume`, `test_gated_takes_priority_over_active`, `test_active_takes_priority_over_pending`, `test_blocked_phase_not_returned` (locks current behaviour with explicit rationale), `test_real_resume_via_state_md_round_trip` (end-to-end: write SessionState with phase_2 ACTIVE → start_session → decompose_plan → get_current_phase returns phase_2 ACTIVE). Test count 738 → 743 + 7 skipped.

**Rationale:** Investigation triggered by user pushback on the prod-001 fix. After PR #58 corrected the prod-001 STATE.md from `phase_3: prep_complete` to `phase_3: active`, `zo continue` printed "All phases complete. Nothing to do." instead of resuming Phase 3. Tracing through the call stack revealed that `get_current_phase` only returned GATED or PENDING phases — ACTIVE was silently skipped. But ACTIVE is a real, persisted state set by two production paths:

- `apply_human_decision(GateDecision.ITERATE)` at orchestrator.py:707 — when a human says "iterate" at a gate, the phase is reset to ACTIVE and subtasks are cleared for the next iteration.
- The autonomous experiment loop at orchestrator.py:1046 — when the loop verdict is CONTINUE (target not hit, budget remains, no plateau, no dead-end), the phase is reset to ACTIVE so the next iteration mints a child experiment.

If a session ends gracefully, the next-iteration prompt is built and the phase status is consumed within the same session. But if the session is interrupted (Ctrl-C, SSH disconnect, OS reboot, agent crash) BETWEEN `phase.status = PhaseStatus.ACTIVE` and the next iteration's launch, the ACTIVE status persists in STATE.md. On `zo continue`, the orchestrator's `_restore_phase_states` correctly reads "active" back into the phase, but `get_current_phase` then drops it on the floor.
...
1. **Treat ACTIVE-on-disk as an error** (raise on resume, force the user to manually correct STATE.md). Rejected — the autonomous loop legitimately sets ACTIVE between iterations, so a graceful interruption is a normal scenario, not a corruption case.
2. **Add a session-lock so ACTIVE means "another process is on it, don't double-run"**. Rejected — overkill for the failure modes we actually have, and ZO is single-session-per-project by design (PRD §7 v1 scope).
3. **Bundle BLOCKED into the resume path too**. Rejected — BLOCKED requires human escalation per `apply_human_decision(GateDecision.ESCALATE)`. Auto-resuming a BLOCKED phase would silently bypass the escalation. Better to leave BLOCKED out and let the CLI surface it through a separate path if needed.
4. **Return the prod-001 STATE.md to `phase_3: pending` instead and call it done**. Rejected — that's the workaround, not the fix. The user's pushback ("why is this even an issue") was about the platform behaviour, not the data. Fixing `get_current_phase` makes the platform robust to interruption regardless of what STATE.md contains.

**Outcome:** `src/zo/orchestrator.py` (+11 lines including expanded docstring), `tests/unit/test_orchestrator.py` (+87 lines: 5 new tests in `TestGetCurrentPhase`). Test count 738 → 743 + 7 skipped. ruff `src/zo/orchestrator.py tests/unit/test_orchestrator.py` clean. PR #64 (originally scoped to parse-time validation) extended to cover both fixes — same investigation, same domain (STATE.md → orchestrator state-machine), same root concern (input/state-machine assumptions narrower than the failure modes that actually occur). PR-037 added to PRIORS.md. Production STATE.md (PR #58) updated to `phase_3: pending []` — the semantically correct value for prod-001 (ZO has not actually started phase_3 work yet; the prep wave was preparatory infrastructure, not phase_3 subtasks). With the orchestrator fix `phase_3: active` would also resume cleanly, but `pending []` is the more accurate data-model value.

---

## Decision: 2026-05-05T12:00:00Z
**Type:** ROADMAP + ARCHITECTURE
**Title:** Q3/H2 roadmap finalised from external user feedback — three workstreams (cost, cross-phase autonomy, accessibility); architecture decisions on UI surface, multi-provider scope, caveman integration path

**Decision:** Roadmap structured into three tiers based on three pieces of external user feedback received post-session-027. `docs/roadmap.mdx` rewritten (replaces prior 3-card "top priorities" framing); full internal plan archived at `~/.claude/plans/so-before-i-begin-peaceful-valley.md`.

**Tier 1 (next 2-4 weeks):**
1. **Caveman integration ablation** — bundle as scoped Claude Code skill in `--low-token` preset, scoped to agents without structured-artifact requirements (data-engineer, lead summarisation; not oracle-qa, not Phase-4 model-builder which emit `result.md` / `metrics.jsonl` / `training_status.json`). Target: +10-20pp savings on top of the measured ~30%.
2. **Onboarding hardening** — one-line installer (`curl … | bash`), Homebrew formula, new `zo doctor` command extending existing `setup.sh` auto-fix path.

**Tier 2 (1-3 months):**
3. **Cross-phase backedge primitive** — new `RETURN_TO_PHASE` `GateDecision` lets a later-phase agent autonomously request revisit of any earlier phase. Budget guardrails (`--max-cross-phase-revisits N`, default 2), Jaccard-similarity dead-end detection (reused from `experiment_loop._check_dead_end`), revisit-reason payload re-prompts the earlier phase with the new constraint. State-machine: phase N requests revisit of phase M → M flips COMPLETED→ACTIVE, N flips ACTIVE→BLOCKED on M as new dependency. Validates F3 (DAG→graph) hypothesis cheaply before committing to the full graph refactor.
4. **VS Code extension** — chosen over Tauri/Electron from-scratch app and over VS Code fork (Cursor/Windsurf model). Architecture: **headless tmux as engine + xterm.js webview as renderer** (`tmux new-session -d` + `tmux pipe-pane -o 'cat >> .zo/runtime/pane.log'` → file watcher → xterm.js). Sub-agent panes render as separate xterm.js instances. Comms feed reads existing `comms.py` JSONL. Zero changes to `wrapper.py` (which gains a `--headless-tmux` flag). New repo `zo-vscode/` (TypeScript).
5. **ZO Studio bundled installer** (optional follow-on to #4) — one-click platform installer bundling VS Code + ZO extension + Claude CLI + uv. The "Codex desktop" UX without a separate codebase to maintain.
6. **SDK refactor for prompt caching + Batch API + Files API** — pushes savings toward 70-80% target.

**Tier 3 (3-6+ months):**
7. **Full weighted graph architecture** — phases as nodes with weighted reversible edges, `completeness ∈ [0,1]` per node, prioritised revisit queue ranked by `revisit_weight`, `zo nodes freeze <phase_id>` command. Pre-reqs: SDK refactor (cost discipline) + cross-phase backedge primitive (validation).
8. **Multi-provider abstraction** — reframed as **worker-pool model** (Claude Code lead dispatches single-agent jobs to CodeX/OpenCode workers; workers report back via filesystem JSONL), NOT full peer-to-peer team parity. Full parity would require rebuilding Claude Code's native `TeamCreate` / `Agent` / `SendMessage` infrastructure on top of bare APIs.

Recommended sequencing within Tier 2: **architecture-first** (cross-phase backedge primitive → SDK refactor → VS Code extension), with visibility-first as a defensible alternative if researcher onboarding/demo-ability is the priority growth lever.

**Rationale:** The three pieces of feedback hit three distinct growth blockers (cost, cross-phase autonomy, accessibility) — non-overlapping. Tiering reflects effort/risk profile: Tier 1 reuses existing infrastructure (benchmark harness from session-025, `setup.sh` auto-fix from v1.0.2-pre); Tier 2 introduces new architectural primitives but with bounded scope; Tier 3 are major refactors gated behind Tier 2 validation. The VS Code extension (vs fork vs standalone) trade-off is decisive — extension gets file tree, git, integrated terminal, marketplace distribution, webviews, command palette all for free, with no upstream-sync maintenance burden. Cursor/Windsurf require an engineering org just to keep up with VS Code releases; we don't need that. Codex-desktop appeal addressable via the bundled installer follow-on. The headless-tmux + xterm.js architecture preserves all existing `wrapper.py` logic and the lessons from PR-001/PR-022/PR-031 — Claude Code keeps its real TTY, native peer-to-peer comms work as today, sub-agent panes render naturally; UI gets a streaming render with crash-recovery semantics (tmux survives VS Code restarts, reattach on reopen). For F3, the cross-phase backedge primitive is the cheap-validation step before committing to the full graph refactor (user explicitly flagged convergence/token-blowup risk: *"needs careful balancing to not just botch it"*).

**Alternatives considered:**
- **VS Code FORK** (Cursor/Windsurf model) — full branding control but inherits the entire VS Code maintenance burden (security patches, upstream merges, marketplace, code signing, auto-update). Out of scope for a single-developer project.
- **Tauri/Electron from-scratch app** — app shell is the expensive part; VS Code provides it for free with a richer ecosystem.
- **node-pty rewrite (drop tmux entirely)** — Claude Code TUI is finicky (PR-001/PR-022/PR-031 lessons). Replacing tmux with node-pty introduces TTY-related regression risk on the orchestration core. Headless tmux as engine sidesteps this.
- **Caveman drop-in (always-on)** — risk of breaking agents that emit structured artifacts. Scoped activation in `--low-token` preset is safer.
- **Multi-provider full parity (peer-to-peer mesh)** — would require rebuilding the team primitive, message bus, task tracker, lifecycle manager. Worker-pool model captures most of the value with a fraction of the work.
- **F3 full graph refactor without backedge-primitive validation** — convergence/token-blowup risk flagged by user; cheap validation step de-risks the larger commitment.
...
- **`website/src/pages/index.html:281`** (added on a second pass) — §03 headline "Same six phases. *Now in days.*" → "Weeks of work. *Now in days.*". The prior phrasing forward-referenced "six phases" but those phases aren't introduced until §07 (four sections later, after Idea / Handoff / Team), so the reader had to wait through three intermediate sections for the payoff. The §03 lede also only enumerates *five* items ("data engineering, cleaning, scaffolding, eval scripts, packaging") — close to six but not aligned with §07's Data / Features / Model / Training / Analysis / Packaging, so the framing was internally inconsistent. New headline preserves the compression punch (echoes the `lanes-pretitle` "5 working days · compressed from 10 weeks" on the same section) without making promises the section hasn't kept.
...
## Decision: 2026-05-29T10:00:00Z
**Type:** DOCUMENTATION
**Title:** Document `--bypass-permissions` in docs/COMMANDS.md — completing the PR #92 doc cascade

**Decision:** Added `--bypass-permissions` to `docs/COMMANDS.md` (the terminal-command reference) in four places: the `zo build` usage block, a new "Permission prompts:" subsection under `zo build`, the `zo continue` usage block, and the `zo continue` "same semantics as `zo build`" options bullet. The description matches the framing already shipped in `docs/cli/build.mdx`: `--bypass-permissions` auto-approves **every** Claude Code tool-call prompt (Bash/Edit/Write/Read/...) for the run, is independent of `--gate-mode` (which gates ZO's phases, not individual tool calls), is implied by `--gate-mode full-auto`, is off by default, and works in both tmux (temporary `.claude/settings.local.json` overlay, auto-restored on exit) and headless (`--dangerously-skip-permissions`) modes.

**Rationale:** PR #92 (session 031) shipped `--bypass-permissions` and updated `docs/cli/build.mdx` (full "Permission prompts" section + 6-row truth table) and `docs/cli/overview.mdx` (shared options table), but missed `docs/COMMANDS.md` — the reference that already documents the sibling build/continue flags (`--no-tmux`, `--low-token`, `--gate-mode`, and `--no-headlines` from session 032). COMMANDS.md was therefore the one surface where the flag was undiscoverable. Surfaced when the user asked whether the docs covered bypass-permissions; a targeted audit found build.mdx/overview.mdx complete and COMMANDS.md stale. PR-003 cascade discipline: a command's flag set must be reflected in COMMANDS.md.

**Outcome:** 4 edits to `docs/COMMANDS.md` (10 insertions, 5 deletions); no code or tests touched. validate-docs.sh 10 passed / 0 failed / 1 pre-existing warning (client-blocklist skip). Docs + memory only, so the ruff/pytest CI matrix is unaffected by construction (zero `.py` files in the diff). **No new PRIOR** — clean cascade completion, not a failure-driven evolution; the generalisable lesson (PR-003: command flag sets cascade to COMMANDS.md) already exists. **Minor pre-existing drift noted, left out of scope:** README test badge reads 743 vs STATE's 760 post-#92/#93 (validate-docs "within tolerance"). Ships in branch `claude/document-bypass-permissions-commands`.

---

## Decision: 2026-05-30T12:00:00Z
**Type:** FEATURE
**Title:** Batch B+C — training intelligence (training-checker agent, checkpoint cadence + disaster recovery, research-scout general-AI track, auto-maintained experiment checklist)

**Decision:** First of the process-hardening batches requested by the user ("harden the rules I keep repeating so they take effect without me"). Framing is PR-005 applied to the autonomous *run*: turn aspirational agent prose into enforced platform behaviour. Four hardenings shipped in one PR (batches A/D/E queued):

1. **New `training-checker` agent** (`.claude/agents/training-checker.md`, Sonnet, tier phase-in, team project) — a per-model-run live monitor the Lead spawns as `training-{modelname}-checker`. Tails the active experiment's `metrics.jsonl` / `training_status.json`, alerts Model Builder + Lead on NaN/Inf, divergence, gradient blow-up, overfitting, dead LR, or stall (broken run dies early instead of burning the budget), and at run end writes a mechanistic `diagnosis.md` + feeds `next.md`. **Enforcement** is an always-fires instruction in `Orchestrator._prompt_experiment_context` (Phase 4 only) — independent of whether a plan lists the agent in its active set — backed by the agent file and `AGENT_PHASE_MAP["training-checker"] = ["phase_4"]` + role/ownership/off-limits map entries.
2. **Checkpoint cadence + disaster recovery.** New importable `should_checkpoint(epoch, total_epochs, *, every=10, is_best=False)` in `training_metrics.py` replaces the model-builder contract's previously-undefined `should_checkpoint(...)` pseudocode. `model-builder.md` gains a "Checkpointing and Disaster Recovery (REQUIRED)" section: DL = every 10 epochs + best + last with fully-resumable state (model/optimizer/scheduler/AMP-scaler/epoch/RNG) and resume-from-`last.pt`; classical ML = per-CV-fold + best + persist HPO study/search state (Optuna/joblib) + boosting early-stop + wall-clock fallback.
3. **Research-scout general-AI track.** `research-scout.md` gains a Phase-4-iteration section: survey the general ML literature (time-series/sequence modelling, optimization, regularization, architecture) **method-first, not domain-first**, pairing with the training-checker on each flagged failure mode. Additive to its existing client problem-class survey.
4. **Auto-maintained experiment checklist.** `experiments.render_checklist()` / `write_checklist()` generate `.zo/experiments/CHECKLIST.md` deterministically; `write_checklist` is baked into every registry mutation (`mint_experiment`, `update_result`, `update_status`, `update_next_ideas`) so the human-readable log (exp → hypothesis → metric → Δ vs parent → tier → top shortfall, + "Next planned") always reflects the registry without agent action.

**Rationale:** The four gaps were the PR-005/PR-009/PR-035 family one layer down (in the autonomous run): the evidence sweep found no training monitor, `should_checkpoint` referenced-but-undefined, and the lead prompt's "update experiments/README.md index" never executed by any code. The user was manually re-supplying these every run. The fix in each case is enforcement: a code path that always runs (Phase-4 prompt), an importable helper (so the contract can't reference a phantom function), and deterministic platform-side generation (so the checklist isn't delegated to a model that may skip it). The checker's guarantee is the orchestrator prompt rather than the plan's active-agent roster because `_agents_for_phase` only staffs agents a plan lists — too fragile for an always-on safety behaviour.

**Alternatives considered:**
- Dynamic/ephemeral training-checker (no agent file) — rejected: less discoverable, contract would live only in a prompt string and wouldn't be validated by setup.sh; a first-class file is the more "hardened" choice (cost: the 20→21 roster cascade).
- Wire the checklist into the orchestrator gate path instead of registry mutations — rejected: baking `write_checklist` into mint/update_* makes it impossible to mutate the registry without refreshing the checklist, and needs zero orchestrator changes.
