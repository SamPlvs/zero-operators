...
## Decision: 2026-04-25T23:55:00Z
**Type:** ROADMAP
**Title:** Logged future work — `--low-resources` / serial-spawn mode for Macs near `kern.maxprocperuid` cap
**Decision:** Logging (not implementing) a future feature surfaced by the session-022 spawn-crash diagnosis. The friend's reproducer (`zo build plans/mnist-digit-classifier.md`) hits `classical_ml` Phase 1's 5-agent parallel spawn burst (`data-engineer` + `test-engineer` + `research-scout` + `code-reviewer` + `domain-evaluator` — see [src/zo/_orchestrator_phases.py:61-64](src/zo/_orchestrator_phases.py)). On macOS Macs already near the per-UID process cap (default `kern.maxprocperuid=2666`), this burst can push the user over the cap and `fork()` starts failing — which is what crashes Claude Code teammate sessions. The shipping fix for users today is the troubleshooting doc (PR #52: upgrade Claude Code, raise the cap, close Electron apps, try `--no-tmux`). The proposed feature: add a `--low-resources` CLI flag (or a plan-level `## Resources` block) that instructs the Lead Orchestrator to spawn agents **serially within a phase** rather than in parallel — eliminating the burst spike at the cost of roughly 2× wall-clock per phase. Implementation touch points likely: `_prompt_coordination()` in `orchestrator.py` (parallel-vs-serial spawn instruction), CLI flag plumbing in `cli.py` build/continue, optional plan parser support for `## Resources` block. Sam will pick up next session.
**Rationale:** PR-015 deliberately made Phase 1 spawn 5 agents in parallel for production-grade defaults; reverting to fewer agents would degrade quality on real projects. A flag-gated alternative preserves PR-015's defaults while giving resource-constrained users a working escape hatch — and is a more honest position than "ZO works on every Mac" when it demonstrably doesn't on marginal-resource boxes. Logging it here so the design intent + cross-references are preserved across sessions; not implemented yet because the user hasn't confirmed the spawn crash is resolved upstream (Claude Code 2.1.119+) and we want to validate that the troubleshooting-doc mitigations are sufficient for most users before adding ZO-side complexity.
**Outcome:** Item #10 added to STATE.md "What's Next". DECISION_LOG entry preserves design intent. PR #52's branch carries this update. No code change.
