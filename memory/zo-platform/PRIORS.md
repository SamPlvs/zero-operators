...
## PR-040: Training-Time Observability and Checkpoint Cadence Must Be Enforced, Not Left as Aspirational Agent Contracts

**Source:** Session 034 (2026-05-30), Batch B+C — process-hardening from the user's "stop reminding ZO" request
**Root cause category:** incomplete_rule

**Failure:** During autonomous Phase 4 runs the user had to repeatedly re-instruct ZO to (a) monitor training and catch NaN/divergence early, (b) checkpoint at a sane cadence with resumable state, and (c) keep a succinct running experiment log. Investigation found all three were aspirational, not enforced: no training-monitor agent existed; `model-builder.md` called an undefined `should_checkpoint(epoch, val_metrics)` with no cadence defined anywhere; the lead prompt's "update experiments/README.md index after each run" was never executed by any code (`experiments/README.md` was scaffolded and never touched). Same family as PR-005 (aspirational rules without enforcement), PR-035 (aspirational agent contracts get ignored under sub-optimal models), and PR-009 (built modules must be wired).

### Rules

1. **Behaviour that must happen on every run belongs in an enforced code path, not in agent prose.** A monitor that "should" be spawned, a cadence the agent "should" follow, an index the agent "should" update — all degrade to optional the moment a model takes the path of least resistance (PR-035). Put the guarantee where the model can't route around it.
   - **Why:** The Lead's spawn decisions are model-driven, and `_agents_for_phase` only staffs agents a plan explicitly lists. Relying on either for an always-on safety behaviour is fragile.
   - **How to apply:** Enforce via (a) an orchestrator prompt section that ALWAYS fires for the relevant phase (the training-checker spawn instruction lives in `_prompt_experiment_context`, gated only on `phase_id == phase_4`), and/or (b) deterministic platform-side generation (the experiment checklist is regenerated by `write_checklist` inside every registry mutation, never delegated to an agent).

2. **A contract that references a helper must ship the helper.** `model-builder.md` referenced `should_checkpoint(...)` that existed nowhere — a phantom function the agent had to reinvent (inconsistently, or not at all). Shipping an importable `should_checkpoint` in `zo.training_metrics` turns the contract from aspiration into a callable API.
   - **How to apply:** When an agent contract shows code calling a `zo.*` helper, that helper must be importable and covered by a test. Grep new contract code blocks for calls that resolve to nothing.

3. **Deterministic artifacts are generated by the platform, not delegated to agents.** A running experiment log is fully derivable from the registry, so the platform renders it (`render_checklist`); it is not an agent chore. "Agents update the index" instructions are the aspirational-rule anti-pattern — deterministic data deserves a deterministic generator.
