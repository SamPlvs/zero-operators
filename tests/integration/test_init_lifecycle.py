...
"existing prod-001 repo on branch samtukra, training on a remote GPU host,
...
def existing_repo(tmp_path: Path) -> Path:
    ...
    repo = tmp_path / "prod-001-repo"
    repo.mkdir()
    (repo / ".git").mkdir()
    pkg = repo / "src" / "prod-001"
    pkg.mkdir(parents=True)
    (pkg / "__init__.py").write_text("__version__ = '0.1.0'\n")
    (pkg / "trainer.py").write_text(
        "def train():\n    return 'real user code'\n", encoding="utf-8",
    )
    (repo / "README.md").write_text("# prod-001\n", encoding="utf-8")
...
class TestInitLifecycle:
    ...
    def test_full_lifecycle(
        ...
        dry = self._run(
            ...
            "--data-path", "gpu-server-01:/mnt/data/prod-001/f5",
        )
        ...
        assert not (zo_root / "memory" / "prod-001").exists()
        assert not (zo_root / "targets" / "prod-001.target.md").exists()
        assert not (zo_root / "plans" / "prod-001.md").exists()
        # Existing repo NOT touched
        assert not (existing_repo / "configs").exists()
        assert (existing_repo / "src" / "prod-001" / "trainer.py").read_text() \
            == "def train():\n    return 'real user code'\n"
        ...
        assert (zo_root / "memory" / "prod-001" / "STATE.md").exists()
        assert (zo_root / "memory" / "prod-001" / "DECISION_LOG.md").exists()
        assert (zo_root / "memory" / "prod-001" / "PRIORS.md").exists()
        target_text = (zo_root / "targets" / "prod-001.target.md").read_text()
        assert "target_branch: samtukra" in target_text
        assert str(existing_repo) in target_text
        plan_text = (zo_root / "plans" / "prod-001.md").read_text()
        ...
        assert (existing_repo / "src" / "prod-001" / "trainer.py").exists()
        assert (existing_repo / "configs" / "data").is_dir()
        assert (existing_repo / "STRUCTURE.md").exists()
        ...
        target_text2 = (zo_root / "targets" / "prod-001.target.md").read_text()
